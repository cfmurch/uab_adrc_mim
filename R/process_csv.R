...
process_csv <- function(.data_list, entry_form, dict = processing_table_dict){
  ...
  #Step through each folder to process them individually
  dat_proc <- lapply(names(.data_list), function(.uds_set){
    ...
    dat_out <- lapply(.data_list[[.uds_set]], function(.mim){
      ...
      header_curr$uds_version <- .uds_set
  ...
  names(dat_proc) <- names(.data_list)
  return(dat_proc)
}
